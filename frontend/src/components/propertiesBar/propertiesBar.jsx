...
import { Play, Square, Settings, Zap, Database, AlertCircle, Trash2 } from 'lucide-react';
      ...
      <header className="p-6 border-b border-emerald-100/50 bg-emerald-50/30">
        {selectedElement ? (
          ...
          <div className="flex items-center gap-4 opacity-70">
            ...
            <h2 className="font-black text-sm uppercase tracking-tight text-slate-400">Properties</h2>
          </div>
        )}
      </header>
              ...
              return (
                <div key={idx} className="group p-4 bg-white border-2 border-emerald-50 rounded-[22px] hover:border-emerald-200 transition-all shadow-sm">
                  ...
                  <input
                    ...
                    onBlur={(e) => !connected && handleBlurCommit(idx, e.target.value)}
                    onKeyDown={(e) => e.key === 'Enter' && !connected && e.target.blur()}
                    disabled={connected}
          ...
          <div className="p-6">
            <div className="p-4 bg-emerald-50 rounded-[22px] border-2 border-emerald-100">
              <p className="text-xs text-emerald-700">
                <span className="font-black">Source:</span> {selectedElement.source}
              </p>
              <p className="text-xs text-emerald-700 mt-2">
                <span className="font-black">Target:</span> {selectedElement.target}
              </p>
            </div>
          </div>
        ) : (
